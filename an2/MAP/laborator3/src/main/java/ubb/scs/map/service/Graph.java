...
public class Graph {
    ...
    private Map<Long, List<Long>> buildAdjacencyList(Iterable<Prietenie> edges) {
        Map<Long, List<Long>> adjList = new HashMap<>();
        for (Prietenie edge : edges) {
    ...
    public int countConnectedComponents(Iterable<Prietenie> edges) {
        for (Prietenie edge : edges) {
            Long vertex1 = edge.getId().getLeft();
            Long vertex2 = edge.getId().getRight();
    ...
    public Map.Entry<List<Long>, Integer> longestPathInNetwork(Iterable<Prietenie> edges) {
